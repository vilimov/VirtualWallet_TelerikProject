    ...
    public class UsersController : Controller
    {
		...
		public IActionResult Login(LoginViewModel model)
		{
			...
			try
			{
                ...
                var amount = $"No wallet";
                var currency = $"No wallet";
                if (user.Wallet!=null && user.Wallet.IsInactive==false)
                {
                    amount = $"{user.Wallet.Balance:f2}";
                    currency = user.Wallet.CurrencyCode.ToString();
                }
